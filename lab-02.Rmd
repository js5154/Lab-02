...
author: "Team Name: Joe, Borys, Patrick, Kyle"
date: "`r Sys.Date()`"
...
plastic_data_all %>%
  ...
  group_by(region) %>%
  summarise(meanp = mean(total_mismanage_plastic))
...
plastic_data_all %>%
  ...
  mutate(pct_mismanaged_plastic_ocean =
            (mismanaged_plastic_ocean/mismanaged_plastic)) %>%
  group_by(region) %>%
    summarise(med = median(pct_mismanaged_plastic_ocean))
...
plastic_data_all %>%
  ...
  rename(oceanp = mismanaged_plastic_ocean) %>%
  rename(p = mismanaged_plastic) %>%
